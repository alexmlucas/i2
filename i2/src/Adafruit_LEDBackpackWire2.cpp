...
void Adafruit_LEDBackpack::setBrightness(uint8_t b) {
  ...
  Wire2.write(HT16K33_CMD_BRIGHTNESS | b);
...
void Adafruit_LEDBackpack::blinkRate(uint8_t b) {
  ...
  Wire2.write(HT16K33_BLINK_CMD | HT16K33_BLINK_DISPLAYON | (b << 1));
  Wire2.endTransmission();
}
...
void Adafruit_LEDBackpack::begin(uint8_t _addr = 0x70) {
  ...
  Wire2.beginTransmission(i2c_addr);
  Wire2.write(0x21); // turn on oscillator
  Wire2.endTransmission();
...
void Adafruit_LEDBackpack::writeDisplay(void) {
  Wire2.beginTransmission(i2c_addr);
  Wire2.write((uint8_t)0x00); // start at address $00

  for (uint8_t i = 0; i < 8; i++) {
    Wire2.write(displaybuffer[i] & 0xFF);
    Wire2.write(displaybuffer[i] >> 8);
  }
  Wire2.endTransmission();
}
...
void Adafruit_7segment::writeColon(void) {
  Wire2.beginTransmission(i2c_addr);
  Wire2.write((uint8_t)0x04); // start at address $02

  Wire2.write(displaybuffer[2] & 0xFF);
  Wire2.write(displaybuffer[2] >> 8);
